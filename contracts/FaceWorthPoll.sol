...
contract FaceWorthPoll {
  ...
  function sortParticipants() private view returns (address[]) {
    address[] memory sortedParticipants_ = new address[](revealCount);
  ...
  function getTurningPoint(uint _totalWorth, address[] _sortedParticipants) private view returns (uint) {
    uint turningPoint_;
  ...
  function getTotalWorth() private view returns (uint) {
    uint totalWorth_ = 0;
    for (uint i = 0; i < participants.length; i++) {
  ...
  function getCommitTimeElapsed() external view returns (uint) {
    return (block.number - startingBlock) * 100 / (commitEndingBlock - startingBlock);
  }

  function getRevealTimeElapsed() external view returns (uint) {
    if (block.number < commitEndingBlock) {
      return 0;
    } else {
  ...
  function getNumberOfParticipants() external view onlyInitiator returns (uint) {
    return participants.length;
  }

  function getParticipantsRequired() external view onlyInitiator returns (uint) {
  ...
  function getWorthBy(address _who) external view returns (uint8) {
    require(currentStage == Stage.ENDED);
    return worthBy[_who];
  }
